...
```bash
cp .env.example .env
# Отредактируйте .env (DB_HOST=localhost, S3_ENDPOINT=localhost:9002)
go mod download
go run ./cmd/api
```
...
- **PostgreSQL** (с хоста, pgAdmin): **localhost:5434** → в контейнере порт 5432 (user/pass: postgres/postgres, DB: marketplace)
- **MinIO**: http://localhost:9002 (minioadmin/minioadmin), консоль: http://localhost:9003
