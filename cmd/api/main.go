...
func main() {
	cfg, err := config.Load() // config.Load() calls godotenv for .env in cwd
	if err != nil {
		log.Fatal(err)
	}
	if cfg.OpenAI.APIKey != "" {
		log.Printf("OpenAI: API key loaded (length %d)", len(cfg.OpenAI.APIKey))
	} else {
		log.Println("OpenAI: OPENAI_API_KEY is empty — RAG and POST /api/ai/career/suggest will return 503")
	}
	...
	ragChunkRepo := repository.NewRagChunkRepository(pool)
	openAIClient := &ai.Client{
		APIKey:         cfg.OpenAI.APIKey,
		BaseURL:        cfg.OpenAI.BaseURL,
		ChatModel:      cfg.OpenAI.Model,
	...
	// RAG: первый запуск / старые вакансии — индекс пуст: переиндексация в фоне (нужен OPENAI_API_KEY).
	if strings.TrimSpace(cfg.OpenAI.APIKey) != "" {
		ctxQ, cancelQ := context.WithTimeout(ctx, 20*time.Second)
		chunkN, _ := ragChunkRepo.Count(ctxQ)
		vacN, _ := vacancyRepo.CountAll(ctxQ)
		cancelQ()
		if chunkN == 0 && vacN > 0 {
			log.Printf("RAG: нет чанков в БД, вакансий: %d — запуск фоновой индексации…", vacN)
			full, err := vacancyRepo.List(ctx, repository.VacancyFilter{}, 10000)
			if err == nil && len(full) > 0 {
				queued := append([]model.Vacancy(nil), full...)
				go func() {
					for i := range queued {
						c, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
						_ = ragIndexer.Reindex(c, &queued[i])
						cancel()
					}
					log.Println("RAG: стартовая индексация завершена")
				}()
			}
		}
	}
	...
	vacancyHandler := handler.NewVacancyHandler(vacancyRepo, userRepo, aesKey, ragIndexer)
	...
	// Список и просмотр вакансий без токена (главная, каталог)
	...
	mux.Handle("POST /api/recruiter/rag/reindex-mine", authMiddleware(middleware.RequireRole(model.RoleRecruiter)(http.HandlerFunc(adminRagHandler.ReindexMine))))
