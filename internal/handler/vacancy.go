...
type VacancyHandler struct {
	...
	aesKey      []byte
	ragIndex    *rag.Indexer
}

func NewVacancyHandler(vacancyRepo *repository.VacancyRepository, userRepo *repository.UserRepository, aesKey []byte, ragIndex *rag.Indexer) *VacancyHandler {
	return &VacancyHandler{vacancyRepo: vacancyRepo, userRepo: userRepo, aesKey: aesKey, ragIndex: ragIndex}
}
...
func (h *VacancyHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if h.ragIndex != nil {
		if v, err := h.vacancyRepo.GetByID(r.Context(), id); err == nil {
			h.ragIndex.ScheduleReindex(v)
		}
	}
