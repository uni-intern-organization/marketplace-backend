...
// CountAll returns the number of rows in vacancies.
func (r *VacancyRepository) CountAll(ctx context.Context) (int64, error) {
	var n int64
	err := r.pool.QueryRow(ctx, `SELECT COUNT(*) FROM vacancies`).Scan(&n)
	return n, err
}
